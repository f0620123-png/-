...
export async function POST(req: Request) {
  try {
    ...
    if (!t) return Response.json({ error: "Missing text" }, { status: 400 });

    const model = process.env.OPENAI_IMAGE_MODEL || "gpt-image-1";

    const prompt = [
      "請依下列穿搭建議，生成一張『全身人物穿搭示意圖』。",
      "風格：電商攝影感、柔光、背景乾淨（米白/淺灰）。",
      "限制：不要文字、不要 Logo、不要水印。",
      "",
      "穿搭建議：",
      t
    ].join("\n");

    const img = await openai.images.generate({ model, prompt, size: "1024x1024" });
